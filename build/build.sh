...
#     eg: "v1.1.1" will name the bundle "streets_of_rage_apworld-v1.1.1"
...
function mk_apworld() {
  ...
  echo "From: ${root}"
  echo "To: ${destdir}"
  mkdir --parents "${destdir}/streets_of_rage"
  rsync --progress \
    --recursive \
    --prune-empty-dirs \
    --exclude-from="${CWD}/apworld.ignore" \
    "${root}/" "${destdir}/streets_of_rage"

  echo "${tag}" >"${destdir}/streets_of_rage/version.txt"
...
function main() {
  ...
  local bundle_base="streets_of_rage_apworld"
